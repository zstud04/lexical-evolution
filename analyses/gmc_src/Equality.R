...
library(KernSmooth);library(ks); library(rgl)
library(mvtnorm); library(misc3d); library(MASS)
library(mvoutlier); library(outliers)

##  When both the sample size and GMC is small, we use Hpi.diag or Hbcv.diag or Hpi.diag to select the bandwidth
##  When the sample size or GMC is not small, we use Hlscv to select the bandwidth
##  out is also used to select the bandwidth
##  xxi and yyi are data. Before we use the following the function gmcPvalue, 
##  we at best rescale xxi and yyi and keep xxi and yyi with similar variances. 
...
gmcPvalue=function(xxi, yyi, out){
  ...
  denote1=min(max((temp-(ybar+h20*EK1)^2)/(yvar+h20^2*EK2), 0), 0.99)
  #h20=h10;
  rr=seq(1,n,1) 
  for (j in 1:n)
  { 
  ...
  tt=(store1)^2/store2; temp0=c(na.omit(tt));  temp0=temp0[temp0>0]
  temp=mean(temp0)*length(temp0)*(x[m]-x[1])/m
  denote2=min(max((temp-(ybar+h20*EK1)^2)/(yvar+h20^2*EK2), 0), 0.99)
  #h20=h10
  rr=seq(1,n,1) 
  for (j in 1:n)
  { 
  ...
  A[3,1]=0-(temp-(ybar)^2)/(yvar+h20^2*EK2)^2
  A[4:6,1]=c(0,0,0)
  tt20=(temp-(ybar)^2)*(1/(yvar+h20^2*EK2)-1/yvar)
  ...
  SA=t(A)%*%Sigma%*%A
  CI3=sqrt(n)*(denote1-tt10-denote2+tt20)/sqrt(c(t(c(1,-1))%*%(t(A)%*%Sigma%*%A)%*%c(1,-1)))
  pvalue=2*(1-pnorm(abs(CI3),0,1))
  return(c(denote1, denote2, pvalue, tt10, tt20))
  #return(c(tt10,tt20,pvalue))
  #return(pvalue)
}
